...
export function VideoCard({ video }: VideoCardProps) {
  return (
    <Link
      href={video.href}
      className="group relative block aspect-[9/16] min-h-[360px] overflow-hidden rounded-xl border border-white/20 bg-black"
    >
      <Image
        src={video.image.src}
        alt={video.image.alt}
      ...
      <div className="absolute bottom-4 left-4 right-4 flex items-end gap-3 text-white">
        <span className="font-[var(--font-serif-brand)] text-3xl leading-none text-[#b8c5a8] lg:text-2xl">
          {video.number}
        </span>

        <h3 className="max-w-[190px] text-base font-semibold leading-tight lg:text-sm">
          {video.title}
        </h3>
      </div>
    </Link>
